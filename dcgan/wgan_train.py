...
def compute_gradient_penalty(D, real_samples, fake_samples, labels):
    ...
    d_interpolates = d_interpolates.reshape(-1, 1)
...
for epoch in range(opt.n_epochs):
    for i, (coords, labels) in enumerate(dataloader):
        ...
        labels = labels.repeat(1,1,coord_shape[1],coord_shape[2])
        ...
        if i % opt.n_critic == 0:
            ...
            # Generate a batch of images
            gen_labels = Variable(FloatTensor(np.random.normal(loc=perf_mean, scale=perf_std, size=(batch_size, opt.n_classes,1,1))))
