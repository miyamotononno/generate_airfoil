...
fixed_z_, fixed_y_label_ = Variable(cuda(fixed_z_)), Variable(cuda(fixed_y_label_))
...
for epoch in range(train_epoch):
    ...
    y_real_, y_fake_ = Variable(cuda(y_real_)), Variable(cuda(y_fake_))
    cnt = 0
    for x_, y_ in train_loader:
        ...
        y_fill_ = fill[y_]
        x_, y_fill_ = Variable(cuda(x_)), Variable(cuda(y_fill_))
        ...
        y_label_ = onehot[y_]
        y_fill_ = fill[y_]
        z_, y_label_, y_fill_ = Variable(cuda(z_)), Variable(cuda(y_label_)), Variable(cuda(y_fill_))
